...
class ytfEncoder(nn.Module):

    def __init__(self):

        super(ytfEncoder, self).__init__()
        ...
        self.eConv3 = nn.Conv2d(in_channels = 64, out_channels = 128, kernel_size = 7, stride = 2, padding = 1)
...
class ytfDecoder(nn.Module):

    def __init__(self):

        super(ytfDecoder, self).__init__()
        ...
        self.dConv2 = nn.ConvTranspose2d(in_channels = 64, out_channels = 32, kernel_size = 7, stride = 2)
        
        ## Apply Instance Normalization.
        self.dBn1 = nn.InstanceNorm2d(num_features = 32)

        ## Convolutional layer with 1 4*4 filter.    
        self.dConv1 = nn.ConvTranspose2d(in_channels = 32, out_channels = 3, kernel_size = 5, stride = 2, padding = 1)
...
class ytfAutoEncoder(nn.Module):

    def __init__(self, Encoder, Decoder):

        super(ytfAutoEncoder, self).__init__()
